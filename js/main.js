...
function playPluck(freq, volume = 0.5) {
  const ctx = getAudio();
  if (!ctx) return;
...
function hueToColor(hue, sat, light) {
  return `oklch(${light} ${sat} ${hue})`;
}

function buildPath(width, height, y0, mouse, pluck, t, ambient, hoverRadius) {
  const N = 64;
  const pts = [];
...
function stringYs(h, n) {
  const margin = h * 0.12;
  const usable = h - margin * 2;
  return Array.from({ length: n }, (_, i) => margin + (usable * i) / (n - 1));
}

function stringThickness(i, scale) {
  return (1.4 + (6.5 - 1.4) * (i / 5)) * scale;
}

// ─── GuitarStringsBg Component ────────────────────────────────

function GuitarStringsBg({ audioOn, hoverRadius = 130, thicknessScale = 2 }) {
  const wrapRef = React.useRef(null);
  const [size, setSize] = React.useState({ w: 0, h: 0 });
  const mouseRef = React.useRef(null);
  const lastMouseRef = React.useRef(null);
  const lastPluckTimeRef = React.useRef(STRINGS.map(() => 0));
  const plucksRef = React.useRef(STRINGS.map(() => null));
  const sparklesRef = React.useRef([]);
  const pathRefs = React.useRef([]);
  const sparkleLayerRef = React.useRef(null);
  const audioOnRef = React.useRef(audioOn);
  ...
  const triggerPluck = React.useCallback((idx, x, y, intensity = 1) => {
  ...
  return (
    <div ref={wrapRef} className="gs-wrap" onClick={handleClick}
      ...
      <svg className="gs-svg" width={size.w} height={size.h}>
        <defs>
          {STRINGS.map((s, i) => (
...
function App() {
  ...
  return (
    <>
      <GuitarStringsBg audioOn={audioOn} hoverRadius={130} thicknessScale={2} />
...
ReactDOM.createRoot(document.getElementById('guitar-root')).render(<App />);
